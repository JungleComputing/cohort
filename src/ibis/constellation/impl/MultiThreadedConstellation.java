...
public class MultiThreadedConstellation {
    ...
    void performSend(Event e) {
    	...
    	handleEventMessage(new EventMessage(identifier, e.target.getOrigin(), e));
    }
...
*/   
    ...
    synchronized ActivityIdentifierFactory getActivityIdentifierFactory(ConstellationIdentifier cid) {
...
*/
 ...
 /*   			cid = deliverLocally(cid, m);
    			...
    			if (cid != null) { 
    				logger.error("INTERNAL ERROR: activity " + m.event.target 
...
*/ 		
    ...
    boolean handleStealReply(SingleThreadedConstellation src, StealReply m) { 
    	...
    	logger.error("ERROR: received steal reply for unknown target " + m.target + " (reclaiming work and dropping reply)");
    	return false;
    }
    ...
    ActivityRecord [] handleStealRequest(SingleThreadedConstellation c) {
        ...
        if (Debug.DEBUG_STEAL) { 
            logger.info("M STEAL REQUEST from child " + c.identifier() + " with context " + context + " to pool " + pool);
        }
        
    	// First attempt to satisfy the request locally without waiting for anyone     	
    ...
    synchronized void register(SingleThreadedConstellation cohort) throws Exception {
...
*/
    ...
    void deliverStealRequest(StealRequest sr) {
    	...
    	for (int i=0;i<workerCount;i++) { 
    		...
    		if (sr.pool.overlap(p)) { 
    			...
    			if (result != null) { 
    				...
    				if (!parent.handleStealReply(new StealReply(identifier, sr.source, result))) { 
    					tmp.reclaim(result);
    				}
    	...
    	// fulfill this request in the first place! Let's send an empty reply....    	
    	parent.handleStealReply(new StealReply(identifier, sr.source, (ActivityRecord) null));
    }
    ...
    void deliverStealReply(StealReply sr) {
        ...
        if (b == null) { 
        	logger.error("ERROR: reveived steal reply for unknown target " + sr.target + " (selecting random target)");
