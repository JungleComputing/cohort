...
public class MultiThreadedConstellation {
	...
	for (int i = 0; i < workerCount; i++) {
		...
		int size = tmp.attemptSteal(result, context,
			c.getConstellationStealStrategy(), pool,
			c.identifier(), localStealSize, true);
	...
	if (map.size() == 0) {
	    ...
	    UnitWorkerContext[] contexts = map.values().toArray(
		    new UnitWorkerContext[map.size()]);
	...
	if (cidFactory.isLocal(cid)) {
	    ...
	    // System.out.println("Forwarding message to new location (relocated): "
	    // + cid);
	    ...
	    // System.out.println("Forwarding message to new location (exported): "
	    // + cid);
