...
public class SingleThreadedConstellation extends Thread {
	...
	restricted = new SmartSortedWorkQueue("ST(" + identifier
		+ ")-restricted");
	 ...
	 * 
	 * logger.warn("SingleThreaded: sleepTime set to " + sleepTime +
	 * " ms.");
	 */
    ...
    ActivityRecord[] attemptSteal(WorkerContext context, StealStrategy s,
	    StealPool pool, ConstellationIdentifier src, int size, boolean local) {
	...
	if (local) {
	    ...
	    if (offset < size) {
		offset += restricted.steal(context, s, tmp, offset, size
			- offset);
	    }
