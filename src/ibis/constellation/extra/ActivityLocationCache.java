...
public class ActivityLocationCache {
	...
	// The assumption is that there won't be (much) contention on adding an entry, but
	// there will be contention on the lookup. Therefore the register method may contain 
	// an expensive lock, while the lookup method uses a map that allows concurrent access.  
	...
	public ActivityLocationCache(int maxSize) {
		...
		locations = new ConcurrentHashMap<ActivityIdentifier, ConstellationIdentifier>(maxSize);
	}	
